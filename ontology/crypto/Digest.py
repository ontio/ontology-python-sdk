...
class Digest(object):
    ...
    @staticmethod
    def hash256(byte_msg, is_hex=False):
        digest = Digest.sha256(Digest.sha256(byte_msg), is_hex)
        return digest

    @staticmethod
    def hash160(byte_msg, is_hex=False):
        digest = Digest.ripemd160(Digest.__sha256(byte_msg), is_hex)
        return digest
