...
def new_registry_ontid_transaction(ontid,pubkey,gas_limit,gas_price):
    ...
    invoke_code = build_neo_vm.build_native_invoke_code(contract_address, bytes([0]), "regIDWithPublicKey", args)
...
def new_get_ddo_transaction(ontid:str):
    ...
    invoke_code = build_neo_vm.build_native_invoke_code(contract_address, bytes([0]), "getDDO", args)
    unix_timenow = int(time())
