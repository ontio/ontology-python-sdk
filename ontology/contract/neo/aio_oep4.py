...
from ontology.contract.neo_vm import Oep4
...
class AioOep4(Oep4):
    ...
    async def symbol(self):
        ...
        response = await self._sdk.default_network.send_raw_transaction_pre_exec(tx)
        return Data.to_utf8_str(response.get('Result', ''))
    ...
    async def init(self, payer: Account, gas_price: int, gas_limit: int):
    ...
    async def transfer_multi(self, transfer_list: list, payer_acct: Account, signers: list, gas_limit: int,
                             gas_price: int) -> str:
        """
        ...
        tx_hash = await self._sdk.default_network.send_raw_transaction(tx)
        return tx_hash
