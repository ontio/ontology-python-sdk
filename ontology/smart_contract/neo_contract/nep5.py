...
class Nep5(object):
    ...
    def __send_init(self, acct: Account, payerAcct: Account, gas_limit: int, gas_price: int, pre_exec=False):
        ...
        if pre_exec:
            ...
            if int(obj["Result"]) is not 1:
                raise Exception("sendRawTransaction PreExec error:", obj)
            return obj["Gas"]
        if acct is None or payerAcct is None:
            raise Exception("acct or payerAcct should not be None")
        return self.__sdk.neo_vm().sendTransaction(bytearray(self.contract_addr), acct, payerAcct, gas_limit, gas_price, func, pre_exec)
